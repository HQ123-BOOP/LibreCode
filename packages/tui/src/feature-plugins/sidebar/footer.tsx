  ...
  return (
    <box gap={1}>
      <Show when={show()}>
        ...
        >
          ...
          <box flexGrow={1} gap={1}>
            ...
            <text fg={theme().textMuted}>{t("sidebar.noProvider")}</text>
            <text fg={theme().textMuted}>{t("sidebar.providerHint")}</text>
            <box flexDirection="row" gap={1} justifyContent="space-between">
              <text fg={theme().text}>{t("sidebar.connectProvider")}</text>
              <text fg={theme().textMuted}>{t("sidebar.commandHint")}</text>
            </box>
          </box>
        </box>
      </Show>
      ...
      <text fg={theme().textMuted}>
        ...
        <b>Code</b> <span>{props.api.app.version}</span>
      </text>
    </box>
  )
}
