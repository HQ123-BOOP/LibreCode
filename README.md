...
LibreCode also ships a desktop application (source in `packages/desktop`). Prebuilt desktop installers are not published in this fork's releases yet; build it from source with `bun run package` in `packages/desktop`.
